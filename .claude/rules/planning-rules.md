...
*   **テスト計画 (Unit & Integration)**:
    *   **Unit Tests**: テーブル駆動テスト (`tests := []struct{...}`) のケース設計。モックが必要な依存関係。
    *   **Integration Tests**: `features/{feature}/integration/` 配下のどのファイルに追加するか。
    *   **統合テスト必要性の判断基準**: 以下のいずれかに該当する場合、統合テストは**必須**です。
        *   バス経由のモジュール連携を行う
        *   HTTP エンドポイントを公開する
        *   複数ゴルーチン／ワーカーの協調を検証する
        *   CLI として利用者に提供される
...
*   **Backend Track**:
    *   Unit: `scripts/process/build.sh`（単体テスト + `go vet`。`features/*/integration/` は除外）
    *   Integration: `scripts/process/integration_test.sh --categories "..."`
    *   Race: `scripts/process/integration_test.sh --race --categories "..."`
...
*   **利用可能なカテゴリ**: `vram`, `monitor`, `bus`, `stats`, `lifecycle`, `palette`
    ...
    **Note**: Do not use `cd` commands in plans. Use paths relative to the project root.
    ...
    > *Exception*: Do not invent `go test -race` in plans; use `integration_test.sh --race` instead.

### 3.2 テストファイルの配置と命名

*   **Backend**: `*_test.go` next to the package (Unit), `features/{feature}/integration/*_test.go` (Integration)
*   カテゴリは `integration_test.sh` 内のファイル対応表で定義する（ビルドタグは用いない）
...
*   **推奨記述例**:
    1.  **Automated Verification**: Run `./scripts/process/build.sh && ./scripts/process/integration_test.sh --categories "vram" --specify "TestPageSize"` to verify the change.
    ...
    *   **Prohibited Commands**:
        ...
        *   ❌ `go build`, `go test`（`-race` 含む。`--race` フラグ付きスクリプトを使うこと）
        *   ❌ `cd ... && ...`
        *   ❌ `integration_test.sh` (STRICTLY PROHIBITED without preceding `build.sh`)
        *   ❌ `build.sh --skip-frontend`, `build.sh --skip-etc`, `build.sh --backend-only`（存在しない）
    *   **Required Commands**:
        *   ✅ `./scripts/process/build.sh`
        *   ✅ `./scripts/process/build.sh && ./scripts/process/integration_test.sh --categories "..."`

    > [!WARNING]
    > **Partial Build Flags in Final Verification**:
    > 最終検証コマンドには `./scripts/process/build.sh`（フラグなし）を使用してください。
    > 開発中の再実行では `--keep-going` のみ任意で付けてよい。
