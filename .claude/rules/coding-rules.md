...
#### Backend (Go)
*   **単体テスト (Unit Test)**:
    *   対象: `cmd/`, `internal/`, `arcproto/` 内の各パッケージ。
    *   実行: `scripts/process/build.sh`。
    *   制約: **Fail Fast**。外部通信を行わず、モックを使用すること。`features/*/integration/` は含めない。
*   **統合テスト (Integration Test)**:
    *   対象: `features/{feature}/integration/` ディレクトリ配下。
    *   実行: `scripts/process/integration_test.sh --categories "..."`。
    *   制約: 実バス・実モジュール連携で検証する。

#### Frontend (VSCode/TypeScript)
*   本リポジトリ（Neurom）には Frontend / VSCode 拡張のテスト対象は無い。
*   将来追加する場合は別仕様で配置とランナーを定義すること。
