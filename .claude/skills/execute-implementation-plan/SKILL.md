...
1.  **既存インフラの確認**:
    *   `features/*/integration/` 配下の既存統合テストを確認し、ヘルパー関数を把握する。
    *   新規テストファイルは `integration_test.sh` の `CATEGORY_FILES` に登録する。
2.  **統合テストコードの実装**:
    *   実装計画の Verification Plan に記載されたテストケースをコードとして実装する。
3.  **コミット**:
    *   `git add && git commit` でテストコードをコミットする。
