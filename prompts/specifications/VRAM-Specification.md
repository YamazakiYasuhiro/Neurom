...
- **1 コマンド = 1 メッセージであり、フレーム単位の原子性が無い。**
  ...
  なお表示状態の読み出し自体は、031 で導入した `Snapshot` により
  **寸法とバッファが同一時点のコピーとして一貫して取得できる**。
  モニタは VRAM バッファを直接参照せず、このコピーを受け取る。
  競合は解消済みだが、描画途中のフレームが見える可能性（原子性）は
  025 の担当であり、`Snapshot` では解消しない
